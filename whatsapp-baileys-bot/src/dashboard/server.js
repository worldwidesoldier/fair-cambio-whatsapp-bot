...
class DashboardServer {
  constructor() {
    ...
    // QR code debounce timeout
    this.qrUpdateTimeout = null;
    this.qrCodeLockTime = null;
    this.manualQRRequest = false;
  ...
  setupMiddleware() {
    ...
    this.app.use(cors({
      origin: ["http://localhost:5173", "http://localhost:3000", "http://localhost:3001", "http://localhost:5174"],
      credentials: true,
  ...
  setupRoutes() {
    ...
    // Route to receive updates from the bot
    this.app.post('/api/bot-update', (req, res) => {
      try {
        const { method, data, branchId, branchName } = req.body;

        console.log(`📨 Received bot update: ${method} from ${branchName || 'unknown'}`);
    ...
    this.app.post('/api/bot/generate-qr', (req, res) => {
      try {
        // Marcar como solicitação manual para aceitar novo QR
        this.manualQRRequest = true;
        this.botStatus.qrCode = null; // Limpar QR atual

        console.log('🎯 Solicitação MANUAL de novo QR Code - flag ativada');
  ...
  setupSocketHandlers() {
    this.io.on('connection', (socket) => {
      ...
      // Force send data immediately and repeatedly
      setTimeout(() => {
        console.log('🔄 Force sending branches and rates IMMEDIATELY');
        this.sendBranches(socket);
        this.sendExchangeRates(socket);
      }, 100);

      setTimeout(() => {
        console.log('🔄 Force sending branches and rates after 1s');
        this.sendBranches(socket);
        this.sendExchangeRates(socket);
      }, 1000);

      setTimeout(() => {
        console.log('🔄 Force sending branches and rates after 3s');
        this.sendBranches(socket);
        this.sendExchangeRates(socket);
      }, 3000);
      ...
      socket.on('generateQRRequest', () => {
        try {
          // Marcar como solicitação manual
          this.manualQRRequest = true;
          this.botStatus.qrCode = null; // Limpar QR atual

          console.log('🎯 Solicitação MANUAL de QR via WebSocket - flag ativada');
      ...
      socket.on('bot-generate-qr', () => {
        try {
          ...
          console.log('🎯 Solicitação MANUAL de QR via bot-generate-qr - flag ativada');
  ...
  // Method to update QR code (primeiro QR aceito automaticamente, depois só manual)
  updateQRCode(qrCode) {
    // Se o QR code é igual ao anterior, não enviar
    if (this.botStatus.qrCode === qrCode) {
      return;
    }

    // Se já existe um QR code ativo E não foi solicitação manual, ignorar
    if (this.botStatus.qrCode && !this.manualQRRequest) {
      console.log('🚫 QR Code ignorado - já existe um ativo. Use "Gerar novo QR Code" para alterar.');
      return;
    }

    // Se foi solicitação manual, limpar flag
    if (this.manualQRRequest) {
      this.manualQRRequest = false;
      console.log('✅ QR Code manual aceito');
    } else {
      console.log('✅ Primeiro QR Code aceito automaticamente');
    }
    ...
    console.log('🔥 QR Code ÚNICO para dashboard:', qrCode ? 'SIM' : 'VAZIO');
    console.log('📊 QR Code preview:', qrCode ? qrCode.substring(0, 30) + '...' : 'NULL');
    ...
    console.log('📊 QR Code ESTÁVEL emitido - permanece até botão ser clicado');
  }
  ...
  setBotConnected() {
    ...
    this.io.emit('botStatus', this.botStatus);
    console.log('✅ Bot conectado! QR code limpo do dashboard.');
  }
...
Object.assign(module.exports, {
  ...
  sendBranches(socket) {
    try {
      console.log('🚀 Starting sendBranches...');
      const branches = this.getBranchesData();
      console.log('🏢 Raw branches result:', branches);
      ...
      if (branches && Array.isArray(branches)) {
        ...
        console.error('❌ Branches data is not an array:', typeof branches);
      }
    } catch (error) {
      console.error('❌ Error in sendBranches:', error);
    }
  }
}
